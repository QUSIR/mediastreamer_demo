...
static void sender_process(MSFilter * f)
{
	...
	//ZCY: iterate
	if (d->session->iterate_func && d->session->iterate_data)
	{
		iterate_func_t stream_iterate = (iterate_func_t)d->session->iterate_func;
		if(NULL != stream_iterate);
		{
			stream_iterate(d->session->iterate_data);
		}
	}
}
...
static void receiver_preprocess(MSFilter * f){
	ReceiverData *d = (ReceiverData *) f->data;
	d->starting=TRUE;
//	printf("receiver_preprocess\n");
}

static void receiver_process(MSFilter * f)
{
//	printf("receiver_process\n");
	ReceiverData *d = (ReceiverData *) f->data;
	mblk_t *m;
	uint32_t timestamp;
	...
	if (d->starting)
	{
		PayloadType *pt=rtp_profile_get_payload(rtp_session_get_profile(d->session), rtp_session_get_recv_payload_type(d->session));
...
//	printf("receiver_process\n");
	...
	while ((m = rtp_session_recvm_with_ts(d->session, timestamp)) != NULL)
	{
		...
		mblk_set_sequence_number(m, rtp_get_seqnumber(m));
		rtp_get_payload(m,&m->b_rptr);
		ms_queue_put(f->outputs[0], m);
//		printf("receiver_process: rtp_session_recvm_with_ts\n");
	}
	...
	if (1)
	{
		...
		tickCnt ++;
		if (tickCnt >= 1000/f->ticker->interval)    // every 1000 ms
		{
			tickCnt  = 0;
			if (sendSeq != d->session->rtp.snd_seq)
				sendSeq  = d->session->rtp.snd_seq;
			else
			{
				mblk_t *header;
				rtp_header_t *rtp;
				uint8_t payload[4] = {0, 0, 0, 0};
				header = rtp_session_create_packet(d->session, RTP_FIXED_HEADER_SIZE, payload, sizeof(payload));
				header->b_cont = NULL;
				rtp = (rtp_header_t*) header->b_rptr;
				rtp->version = 0;
				rtp->paytype = 1;
				rtp->seq_number --;
				rtp_session_sendm_with_ts(d->session, header, timestamp);
			}
		}
	}
