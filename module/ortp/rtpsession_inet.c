...
static void process_rtcp_packet( RtpSession *session, mblk_t *block ) {
	...
	do{
		...
		else if (rtcp_is_APP(block))   //ZCY: rtx
		{
			char name[] = "NULL";
			uint8_t *databuf = NULL;
			int datalen = 0;
			int subtype = rtcp_APP_get_subtype(block);
			rtcp_APP_get_name(block, name);
			rtcp_APP_get_data(block, &databuf, &datalen);
			//printf("RTCP APP [%s] %d %d\r\n", name, subtype, datalen);
			if (memcmp(name, "DPZH", 4) == 0)
			{
				if (subtype == 0 && databuf != NULL && datalen > 0)
				{
					uint16_t rtx_seq_number = ((uint16_t)databuf[0] << 8) | (uint16_t)databuf[1];
					mblk_t  *mp = session->rtx_send_queue._q_stopper.b_prev;
					while (mp != &session->rtx_send_queue._q_stopper)
					{
						rtp_header_t *rtp = (rtp_header_t *) mp->b_rptr;
						if (rtp->seq_number == rtx_seq_number)
						{
							mblk_t *tmp = copymsg(mp);
							rtp = (rtp_header_t *) tmp->b_rptr;
							rtp->version = 0;
							rtp_session_rtp_send (session, tmp);
							break;
						}
						mp = mp->b_prev;
					}
