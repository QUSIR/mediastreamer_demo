...
void rtp_session_rtp_parse(RtpSession *session, mblk_t *mp, uint32_t local_str_ts, struct sockaddr *addr, socklen_t addrlen)
{
	...
	//ZCY: rtx
	if (rtp->version == 0 && rtp->paytype == session->rcv.pt && ntohl(rtp->ssrc) == session->rcv.ssrc)
	{
		uint16_t rtx_seq_number = ntohs(rtp->seq_number);
		mblk_t  *temp = session->rtx_recv_queue._q_stopper.b_prev;
		while (temp != &session->rtx_recv_queue._q_stopper)
		{
			if (((rtp_header_t*)temp->b_rptr)->seq_number == rtx_seq_number)
			{
				int i;
				remq(&session->rtx_recv_queue, temp);
				freemsg(temp);
				//printf("[RTX] Receive %d\r\n", rtx_seq_number);
				rtp->version = 2;
...
#if 0
//ZCY debug lost packets
if ((ntohs(rtp->seq_number) % 160) == 0)
{
	freemsg(mp);
	return;
}
...
//ZCY debug
{
	...
	timecnt ++;
	if (timecnt==1)
		t0 = session->last_recv_time;
	ortp_error("[%3d] %4d.%03d, %5d, 0x%08x", rtp->paytype, session->last_recv_time.tv_sec-t0.tv_sec, session->last_recv_time.tv_usec/1000, rtp->seq_number, rtp->timestamp);
}
#endif
