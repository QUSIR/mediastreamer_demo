...
//ZCY: crypto
int rtp_session_set_crypto(RtpSession *session, bool_t mode, char *key, int keylen)
{
	if (session == NULL)
		return -1;
	if (mode == TRUE)
	{
		if (key == NULL || keylen != (16+14+8))
			return -1;
		if (session->crypto_ctx == NULL)
		{
			session->crypto_ctx = AES_New();
			if (session->crypto_ctx == NULL)
				return -2;
			AES_SetMode(session->crypto_ctx, AES_MODE_SIC);
		}
		AES_SetKey(session->crypto_ctx, key, 128);
		memcpy(session->crypto_key, key, 16);
		memcpy(session->crypto_salt, key+16, 14);
		memcpy(session->crypto_nonce, key+30, 8);
	}
	else
	{
		if (session->crypto_ctx != NULL)
		{
			AES_Free(session->crypto_ctx);
...
static void MakeIV(unsigned char initvec[16], const unsigned char salt[14], const unsigned char nonce[8], mblk_t *mp)
{
	int i;
	rtp_header_t *rtp = (rtp_header_t *) mp->b_rptr;
	memset(initvec, 0, 16);
	memcpy(initvec, nonce, 8);
	initvec[12] = (unsigned char) (rtp->seq_number >> 8);
	initvec[13] = (unsigned char) (rtp->seq_number >> 0);
	for (i=0; i<14; i++)
		initvec[i] ^= salt[i];
}

//ZCY: rtx
int rtp_session_set_rtx(RtpSession *session, bool_t mode, int retry_limit, int send_cache_time, int recv_cache_time)
{
	if (session == NULL)
		return -1;
	if (mode == TRUE)
	{
		if (retry_limit < 0 || send_cache_time <= 0 || recv_cache_time <= 0)
			return -1;
		session->rtx_mode = TRUE;
		session->rtx_retry_limit = retry_limit;
		session->rtx_send_cache_time = send_cache_time;
		session->rtx_recv_cache_time = recv_cache_time;
	}
	else
		session->rtx_mode = FALSE;
	return 0;
}
...
rtp_session_init (RtpSession * session, int mode)
{
	...
	//ZCY: rtx
	qinit(&session->rtx_send_queue);
	qinit(&session->rtx_recv_queue);
	session->rtx_mode = FALSE;
	session->rtx_retry_limit = 1;
	session->rtx_send_cache_time = 2000;
	session->rtx_recv_cache_time =  400;

	//ZCY: iterate
	session->iterate_func = NULL;
	session->iterate_data = NULL;
}
...
__rtp_session_sendm_with_ts (RtpSession * session, mblk_t *mp, uint32_t packet_ts, uint32_t send_ts)
{
	...
	uint32_t last_ts = session->rtp.snd_last_ts;

	// ZCY: must pull the packet up
	if (mp != NULL && mp->b_cont != NULL)
		msgpullup(mp, -1);

	// ZCY: send NULL rtp packet to keep socket alive when only recv.
	if (mp != NULL)
	{
		rtp = (rtp_header_t*) mp->b_rptr;
		if (rtp->version == 0 && rtp->paytype == 1)
		{
			rtp->seq_number = 0xFFFF;
	...
	//ZCY: crypto
	if (mp != NULL && session->crypto_ctx != NULL)
	{
		unsigned char *payload;
		int length = rtp_get_payload(mp, &payload);
		if (length > 0)
		{
			unsigned char initvec[16];
			MakeIV(initvec, session->crypto_salt, session->crypto_nonce, mp);
			AES_SetEncInitVec(session->crypto_ctx, initvec);
			AES_DataEncrypt(session->crypto_ctx, payload, payload, length);
		}
	}

	//ZCY: rtx
	if (mp != NULL && session->rtx_mode == TRUE)
	{
		mblk_t *m;
		uint32_t cache_ts;
		cache_ts = rtp_session_time_to_ts(session, session->rtx_send_cache_time);
		m = copymsg(mp);
		rtp_putq(&session->rtx_send_queue, m);
		while (1)
		{
			m = peekq(&session->rtx_send_queue);
			rtp = (rtp_header_t *) m->b_rptr;
			if ((uint32_t)(packet_ts - rtp->timestamp) < cache_ts)
				break;
			m = getq(&session->rtx_send_queue);
			freemsg(m);
		}
	}
	...
	if (last_ts == packet_ts)
		return error;
	if (session->mode==RTP_SESSION_SENDONLY) rtp_session_rtcp_recv(session);
	else if (session->flags & RTP_SESSION_RECV_NOT_STARTED)  rtp_session_rtcp_recv(session); //ZCY add for VideoStreamSendOnly
	return error;
}
...
rtp_session_recvm_with_ts (RtpSession * session, uint32_t user_ts)
{
	...
	if(mp != NULL && session->rtx_mode == TRUE)
	{
		mblk_t   *curr,   *next,   *temp;
		uint16_t  currseq, nextseq, tempseq;
		// put newest packet into queue
		rtp_putq(&session->rtx_recv_queue, mp);
		// mark lost packet & send rtx request
		curr = session->rtx_recv_queue._q_stopper.b_next;
		next = curr->b_next;
		while (curr != &session->rtx_recv_queue._q_stopper &&
		       next != &session->rtx_recv_queue._q_stopper)
		{
			currseq = ((rtp_header_t*)curr->b_rptr)->seq_number;
			nextseq = ((rtp_header_t*)next->b_rptr)->seq_number;
			tempseq = currseq + 1;
			if (tempseq != nextseq)
			{
				const int dpzh = (('D' << 24) & 0xFF000000) | (('P' << 16) & 0x00FF0000) | (('Z' << 8) & 0x0000FF00) | ('H' & 0x000000FF);
				uint8_t databuf[4];
				temp = rtp_session_create_packet(session, RTP_FIXED_HEADER_SIZE, NULL, 0);
				temp->b_cont = NULL;
				rtp = (rtp_header_t*) temp->b_rptr;
				rtp->ssrc = session->rcv.ssrc ^ dpzh;//'DPZH';
				rtp->paytype = session->rtx_retry_limit;
				rtp->timestamp = ((rtp_header_t*)next->b_rptr)->timestamp;
				rtp->seq_number = tempseq;
				databuf[0] = (tempseq >> 8) & 0xff;
				databuf[1] = (tempseq >> 0) & 0xff;
				rtp_session_send_rtcp_APP(session, 0, "DPZH", databuf, sizeof(databuf));
				//printf("[RTX] Request %d\r\n", tempseq);
				rtp_putq(&session->rtx_recv_queue, temp);
			}
			curr = curr->b_next;
			next = curr->b_next;
		}
	}
	if (session->rtx_mode == TRUE)
	{
		int n;
		ts	= jitter_control_get_compensated_timestamp(&session->rtp.jittctl, user_ts);
		ts -= rtp_session_time_to_ts(session, session->rtx_recv_cache_time);
		while (1)
		{
			// get oldest packet from queue
			const int dpzh = (('D' << 24) & 0xFF000000) | (('P' << 16) & 0x00FF0000) | (('Z' << 8) & 0x0000FF00) | ('H' & 0x000000FF);
			mp = rtp_getq_permissive(&session->rtx_recv_queue, ts, &n);
			if (mp == NULL)
				break;
			if (((rtp_header_t*)mp->b_rptr)->ssrc != (session->rcv.ssrc ^ dpzh));//'DPZH'))
				break;
			//printf("[RTX] Timeout %d\r\n", ((rtp_header_t*)mp->b_rptr)->seq_number);
			freemsg(mp);
		}
	}
	...
	if (mp != NULL && session->crypto_ctx != NULL)
	{
		...
		if (length > 0)
		{
			unsigned char initvec[16];
			MakeIV(initvec, session->crypto_salt, session->crypto_nonce, mp);
			AES_SetDecInitVec(session->crypto_ctx, initvec);
			AES_DataDecrypt(session->crypto_ctx, payload, payload, length);
		}
	}
...
void rtp_session_uninit (RtpSession * session)
{
	...
	//ZCY: rtx
	flushq(&session->rtx_send_queue, FLUSHALL);
	flushq(&session->rtx_recv_queue, FLUSHALL);
	session->rtx_mode = FALSE;
...
int rtp_get_payload(mblk_t *packet, unsigned char **start){
	...
	if (tmp>=packet->b_wptr){//ZCY fix bug
